package com.italker.common.widget.recycler_gallery;
...
public class GalleryItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private void onSetVerticalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        ...
        int itemNewHeight = parent.getHeight() - (int) ScreenUtils.dipToPx(4 * mPageMargin + 2 * mLeftPageVisibleWidth);

        mItemComusemY = itemNewHeight + ScreenUtils.dpToPx(2 * mPageMargin);


        Log.d("TAG", "itemNewHeight=" + itemNewHeight);

        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int topMargin = position == 0 ? ScreenUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ScreenUtils.dpToPx(mPageMargin);
        int bottomMargin = position == itemCount - 1 ? ScreenUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ScreenUtils.dpToPx(mPageMargin);
    ...
    private void onSetHoritiontalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        int itemNewWidth = parent.getWidth() - ScreenUtils.dpToPx(4 * mPageMargin + 2 * mLeftPageVisibleWidth);
        int itemNewHeight = parent.getHeight();

        mItemComusemX = itemNewWidth + ScreenUtils.dpToPx(2 * mPageMargin);

        Log.d(TAG, "onSetHoritiontalParams -->" + "parent.width=" + parent.getWidth() + ";mPageMargin=" + ScreenUtils.dpToPx(mPageMargin) + ";mLeftVis=" + ScreenUtils.dpToPx(mLeftPageVisibleWidth) + ";itemNewWidth=" + itemNewWidth);

        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int leftMargin = position == 0 ? ScreenUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ScreenUtils.dpToPx(mPageMargin);
        int rightMargin = position == itemCount - 1 ? ScreenUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ScreenUtils.dpToPx(mPageMargin);
